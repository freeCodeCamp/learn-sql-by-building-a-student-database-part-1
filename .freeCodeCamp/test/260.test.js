...
describe('The "majors_courses" table', () => {
  it('should have the correct "course_id" foreign key', async () => {
    ...
    try {
      ...
      const foreignKey = res.rows.filter(row => {
        return row.table_name === 'majors_courses' && row.column_name === 'course_id' && row.foreign_table_name === 'courses' && row.foreign_column_name === 'course_id';
      });
