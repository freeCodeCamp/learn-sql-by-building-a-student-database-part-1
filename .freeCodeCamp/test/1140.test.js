...
describe('You', () => {
  it('should run your script by executing it', async () => {
    ...
    try {
      ...
      assert(res1.rowCount === 7 && res2.rowCount === 17 && res3.rowCount === 28 && res4.rowCount === 31 && lastCommand[0] === './insert_data.sh' && (lastCommand[1] === undefined || lastCommand[1].op === ';') && cwd === '/home/strove/project');
    } catch (err) {
      assert(false);
    } finally {
      await client.end();
