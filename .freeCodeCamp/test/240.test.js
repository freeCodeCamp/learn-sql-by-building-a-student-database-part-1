...
describe('The "students_majors" table', () => {
  it('should have the correct "student_id" foreign key', async () => {
    ...
    try {
      ...
      const foreignKey = res.rows.filter(row => {
        return row.table_name === 'students_majors' && row.column_name === 'student_id' && row.foreign_table_name === 'students' && row.foreign_column_name === 'student_id';
      });
