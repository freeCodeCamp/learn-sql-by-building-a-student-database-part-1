...
describe('You', () => {
  it('should run your script by executing it', async () => {
    ...
    try {
      ...
      assert(res1.rowCount === 3 && res2.rowCount === 3 && res3.rowCount === 4 && res4.rowCount === 4 && lastCommand[0] === './insert_data.sh' && (lastCommand[1] === undefined || lastCommand[1].op === ';') && /\/project$/.test(cwd));
    } catch (err) {
      assert(false);
    } finally {
      await client.end();
